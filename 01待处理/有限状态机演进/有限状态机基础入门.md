...
> **目标**：从 C 语言语法根基出发，理解状态机的每个组成部分、设计动机与裸机实现方式
> **前置知识**：C 语言基本语法、函数指针、struct、枚举、裸机 while(1) 主循环
> **进阶路线**：本笔记掌握后，可继续阅读 [[有限状态机演进详解#1. 全局演进总览|演进路线图]]，了解从朴素 switch 到工业级架构的完整蜕变过程
...
函数指针是状态机的基石。理解它，才能理解[[有限状态机演进详解#2.3 V3 函数指针数组映射调用(查表法)|查表法]]与[[有限状态机演进详解#3.3 V6 使用函数地址作为状态(极致效率)|函数地址即状态]]两种高阶实现方式。
...
在讨论状态切换如何实现之前,必须先厘清一个关键概念——**事件源(Event Source)**。

#### 4.0.1 事件 ≠ 事件源

**事件(Event)**是对状态机而言"发生了什么"的抽象信号,如"短按""超时""数据就绪"。

**事件源(Event Source)**是产生该信号的**物理或逻辑来源**,如"GPIO 中断""定时器溢出""DMA 传输完成回调"。

两者的关系:

```
事件源(谁发的)          事件(发了什么)         状态机(怎么响应)
─────────────         ─────────────          ─────────────
GPIO 中断  ──┐
定时器超时 ──┼──→  EVT_KEY_SHORT  ──→  IDLE 状态处理函数
串口命令   ──┘
```

**同一个事件可以有多个触发源**:短按事件既可以来自实体按键的 GPIO 中断,也可以来自串口发来的遥控命令,甚至可以是另一台状态机的通知。对状态机而言,它只关心"短按事件到了",不关心也不应该关心是谁发的。

#### 4.0.2 为什么要解耦事件源与事件?

| 耦合方式 | 代码表现 | 问题 |
|---|---|---|
| 事件源与事件绑定 | `if (GPIO_Pin_0) evt = EVT_KEY_SHORT;` | 换个引脚就得改状态机代码 |
| 事件源与事件解耦 | `evt = EVT_KEY_SHORT;` (不关心来源) | 状态机代码与硬件无关,可移植 |

**解耦的核心收益**:
1. **可移植**:同一套状态机逻辑,在裸机/RTOS/PC 上都能跑,只需替换事件源层
2. **可测试**:单元测试时直接注入事件,无需模拟硬件中断
3. **可复用**:多个事件源复用同一个状态机(实体按键 + 串口遥控 + 蓝牙 APP)
...
│  事件源层    │     │  事件抽象层  │     │  状态机层    │
│ (硬件相关)   │ ──→ │ (事件枚举)   │ ──→ │ (业务逻辑)   │
│ GPIO/UART/   │     │ EVT_KEY_SHORT│     │ handle_idle() │
│ TIM/BLE      │     │ EVT_TIMEOUT  │     │ handle_on()   │
└──────────────┘     └──────────────┘     └──────────────┘
```

| 解耦方式 | 实现手段 | 适用场景 |
|---|---|---|
| 中断写标志位 | 中断中 `g_event = EVT_X;` | 裸机单事件源 |
| 回调函数 | 注册 `on_key_short()` 回调 | 多事件源、需动态切换 |
| 消息队列 | `queue_send(queue, &evt)` | RTOS、多源高频、需缓冲 |

这三种方式正是下面 4.1~4.3 节要详细展开的内容。
...
> [!CAUTION]
> **裸机铁律**：中断回调里只做两件事——记录事件、设置标志。复杂逻辑推迟到主循环处理。
> 这正是[[有限状态机演进详解#4.1 V7 状态机配合消息队列(异步解耦)|异步消息队列架构]]要解决的核心问题。
...
**更准确的理解**:心跳不是状态机"运行"的必要条件。状态机是**响应式**的——只要有事件投喂,它就工作。心跳的作用是**为那些需要时间维度(超时、去抖、周期采样)的场景提供时间基准**。一个纯事件驱动的状态机(所有事件都由外部中断触发)可以完全不需要心跳。

### 5.2 心跳的来源与触发方式

| 来源 | 触发方式 | 产生的事件类型 | 典型场景 |
|---|---|---|---|
| 硬件定时器中断 | 周期性自动触发 | TICK/超时/采样完成 | 传感器轮询、去抖、呼吸灯、协议超时 |
| 外部中断 | GPIO/UART 边沿触发 | 数据到达/电平变化/按键按下 | 按键、串口帧、总线应答 |
| DMA 传输完成中断 | 硬件自动触发 | 数据就绪 | ADC 批量采样、串口接收完成 |
| RTOS 软件定时器 | 任务上下文回调 | 周期性 TICK | FreeRTOS xTimerCallback |
| 主循环轮询 | while(1) 中主动扫描 | 无显式事件,隐含在函数调用中 | 简单按键检测、ADC 轮询 |

**关键区分**:
- **显式心跳**(定时器中断/RTOS 定时器):有明确的 TICK 事件,状态机可以感知"时间流逝"
- **隐式心跳**(外部中断/DMA):没有周期性 TICK,事件来了就处理,不来就等着
- **伪心跳**(主循环轮询):没有固定周期,处理速度取决于 while(1) 中其他任务的耗时

### 5.3 心跳驱动的常见设计模式

#### 模式一:定时器中断扫描型

最经典的裸机心跳方案。定时器以固定周期(通常 1~10ms)中断,在中断中扫描硬件状态,变化时入队事件。
...
void TIM2_IRQHandler(void)
{
    Key_Scan();       /* 按键扫描+去抖 */
    Adc_Poll();       /* ADC 轮询 */
    Timeout_Check();  /* 全局超时检测 */
}
```

**特点**:所有时间相关逻辑集中在定时器中,主循环只做事件处理。实时性好,但中断负担重,扫描项目多时可能超时。

#### 模式二:主循环轮询型

不使用定时器中断,在 while(1) 中直接调用状态机 Task 函数。

```c
while (1)
{
    Drv_Task(&sensor);   /* 驱动层状态机自己检查硬件+推进 */
    App_Task(&ui_fsm);   /* 应用层状态机处理事件 */
    WDT_Feed();          /* 喂狗 */
}
```

**特点**:极简,无中断,但时间精度取决于 while(1) 的执行周期。适用于对时序要求不高的场景。

#### 模式三:RTOS 任务驱动型

每个状态机(或一组状态机)运行在独立的 RTOS 任务中,使用 `osDelay()` 或 `xQueueReceive()` 的超时机制实现心跳。
...
void SensorTask(void *param)
{
    ...
    {
        event_t evt;
        /* 100ms 超时:既是心跳周期,也是等待事件的最大延迟 */
        if (xQueueReceive(g_queue, &evt, pdMS_TO_TICKS(100)))
        {
            fsm_dispatch(&sensor_fsm, evt);
        }
        else
        {
            /* 超时:等同于一次 TICK 心跳 */
            fsm_dispatch(&sensor_fsm, EVT_TICK);
        }
    }
}
```

**特点**:每个状态机有独立优先级和时间基准,互不干扰。但任务切换有开销,不适合 us 级精度的驱动层状态机。

#### 模式四:混合型(中断捕获 + 主循环处理)

中断只负责捕获硬件事件(边沿、DMA 完成等),主循环负责状态机逻辑。这是裸机项目中**最推荐的方案**。
...
while (1)
{
    Drv_Task(&sensor);   /* 检查 irq_pending,推进状态 */
    App_Task(&ui_fsm);   /* 处理应用事件 */
}
```

### 5.4 开源库中的心跳实例

**回答一个常见疑问:开源库里那些需要轮询或放在中断/任务里的接口,都是状态机心跳吗?**

严格来说,不是所有需要定期调用的接口都是"心跳"。需要区分:

| 类型 | 特征 | 是否心跳 | 例子 |
|---|---|---|---|
| 状态机 TICK | 驱动状态流转,内部有状态变量 | 是 | QP 框架 `QHsm_dispatch()` |
| 协议栈轮询 | 处理协议状态,需周期调用 | 是(协议状态机) | LWIP `ethernetif_input()` |
| 硬件抽象轮询 | 无状态变量,只是读寄存器 | 否(纯 I/O) | 简单 ADC 读取函数 |
| 事件循环 | 取事件→分发,无固定周期 | 否(事件驱动) | LVGL `lv_timer_handler()` |

**具体开源库的心跳方式**:

| 开源库 | 心跳方式 | 周期 | 说明 |
|---|---|---|---|
| **LVGL** | 主循环调用 `lv_timer_handler()` | ≥1ms | 内部维护定时器列表,按需触发刷新。本质是事件循环,不是严格的心跳 |
| **QP/C** | 中断中 `QActive_postISR()` 或任务中 `QHsm_dispatch()` | 按需 | 事件驱动,无固定 TICK。主动对象(AO)在 RTOS 任务中等待事件 |
| **LWIP** | 主循环调用 `ethernetif_input()` 或网卡中断 | 按需/中断 | 协议栈内部有 TCP 状态机,超时由 `sys_timeout()` 管理 |
| **FreeRTOS+TCP** | IP 任务中 `xQueueReceive()` | 按需 | TCP 状态机在 IP 任务内运行,事件驱动 |
| **MultiButton** | 主循环调用 `button_handler()` | 5~10ms | 典型轮询去抖,每次调用推进一次状态机 |
| **Protobuf-c** | 无心跳 | N/A | 纯编解码库,无状态,无需心跳 |

**规律总结**:
- **有内部状态机的库**(LVGL 控件、LWIP TCP、MultiButton)→ 需要定期调用或事件驱动,调用周期即是心跳
- **无状态的库**(编解码、CRC、数学运算)→ 无需心跳
- **事件驱动的框架**(QP、FreeRTOS+TCP)→ 不需要固定周期 TICK,事件来了就处理;但超时类状态机仍需定时器

### 5.5 心跳周期如何选择?

| 应用场景 | 推荐周期 | 依据 |
|---|---|---|
| 按键去抖 | 5~10ms | 人手按键抖动 <10ms |
| 传感器轮询(DHT11) | 1000~2000ms | DHT11 采样间隔 ≥1s |
| 电机控制 FOC | 10~50us | PWM 周期决定 |
| UI 刷新 | 16~33ms | 30~60fps |
| 协议超时检测 | 协议规定值的 2~3 倍 | I2C:25ms; MODBUS:100ms |
| 看门狗喂狗 | WDT 超时的 1/2 | 留足裕量 |

**核心原则**:心跳周期 = 最短时间需求。周期过短浪费 CPU,过长则漏检事件。
...
- [[有限状态机演进详解#1. 全局演进总览|演进路线图]] — 八个版本从朴素 switch 到工业级架构的宏观演进路线
- [[有限状态机演进详解#3.1 V4 增加状态进入退出处理(Enter/Exit)|Enter/Exit 生命周期]] — 本笔记第 6 节状态切换流程的工程级实现，解决状态残留问题
- [[有限状态机演进详解#3.2 V5 对状态机进行封装复用(面向对象)|面向对象封装]] — 本笔记第 7 节数据结构演进中"结构体封装"的详细实现与内存全景图
- [[有限状态机演进详解#4.1 V7 状态机配合消息队列(异步解耦)|异步消息队列]] — 本笔记第 4.3 节消息队列模型的完整工程实现与风险分析
- [[驱动层状态机实操指南#1. 核心思想:时序图 → 状态图|驱动层状态机]] — 时序图→状态图，中断解耦，回调通知
- [[应用层状态机实操指南#1. 核心思想:业务流程 → 状态图|应用层状态机]] — 业务流程→状态图，ENTRY/EXIT 消除重复，多机协作
