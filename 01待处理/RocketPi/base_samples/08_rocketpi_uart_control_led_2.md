...
> [[08_rocketpi_uart_control_led|08 主笔记]] 按代码顺序精读了模块；这篇反着来——**假装代码还没写，只看需求，推演每个数据结构和 API 为什么长这样**。目标是使读者具备独立设计同类方案的能力（第一套 08 手写、第二套 09 cJSON）。
...
数一遍 08 的静态函数，按职责归类后其实就六种角色。设计任何命令交互系统时，可以按这六种角色规划函数清单：
...
每个函数只调用它右边的角色。检查设计是否合格，就看**有没有逆向依赖**：解析函数里发错误消息（解析→传输）就是逆向，错误信息应该塞进中间表示或错误参数，由最外层统一汇报。08 用返回值隐式约定、09 的 `const char **error_msg` 出参显式收集错误原因、由 process 函数统一上报，就是这个用意。
...
这十条就是 08 全部代码的骨架。答得上这十问，09 的 cJSON 版就只是"第 3 条换了个实现"。

## 关联笔记

- [[08_rocketpi_uart_control_led]] — 主笔记：逐模块精读与三层流水线
- [[08_rocketpi_uart_control_led_1|串口交互终端的意义]] — 为什么要做这套交互
- [[08_rocketpi_uart_control_led_3|静态辅助函数逐类精读]] — 四类函数逐行拆解
- [[08_rocketpi_uart_control_led_5|函数协作全景]] — 这些函数如何咬合成一次完整交互
- [[08_rocketpi_uart_control_led_6|同类需求识别与设计迁移]] — 把设计方法复制到新需求
